export default class TmdbАpiService {
  ...
  _apiBase = 'https://api.themoviedb.org/3/search/movie?include_adult=false&language=en-US'
  ...
  async getResource(movieTitle, page) {
    ...
    const res = await fetch(`${this._apiBase}&query=${movieTitle}&page=${page}`, options)
  ...
  async rateMovie(movieId, guestSessionId, rating) {
    ...
    try {
      const response = await fetch(`${url}?api_key=${this._apiKey}&guest_session_id=${guestSessionId}`, options)
